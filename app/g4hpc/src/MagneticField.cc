...
void MagneticField::GetFieldValue(const double point[3], 
					double *bField) const
{
  //default zero field
  ...
  if(IsDefined(point[2],rho)) GetVolumeBaseBfield(point, bField);
}
...
void MagneticField::ReadFieldMap(const char* filename) {
  ...
  if (ifile.is_open()) {
    ...
    G4cout << "g4hpc ... Loading magnetic field map: " << filename << G4endl;
	...
	G4Exception("MagneticField::ReadFieldMap()", "BadIndex",
    ...
    G4Exception("g4hpcMagneticField::ReadFieldMap()", "BadFile",
		FatalException, "Invalide Magnetic Field Map!");
  }
}  
