...
void TrackingAction::PreUserTrackingAction(const G4Track* track)
{
    ...
    if (pid == 22)
    {
        fPhotonStack.push_back({ParticleId{pid},
    ...
    if (pid == 11)
    {
        fElectronStack.push_back({ParticleId{pid},
                                  units::MevEnergy{track->GetKineticEnergy()},
                                  {pos.x(), pos.y(), pos.z()},
                                  {dir.x(), dir.y(), dir.z()},
                                  EventId{0},
